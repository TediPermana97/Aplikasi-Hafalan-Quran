...
<head>
  ...
  if (datefield.type!="date"){ //if browser doesn't support input type="date", initialize date picker widget:
      jQuery(function($){ //on document.ready
        $('#awal_bimbingan').datepicker({ dateFormat: 'yy-mm-dd' });
        $('#akhir_bimbingan').datepicker({ dateFormat: 'yy-mm-dd' });
      })
  }
</script>
    ...
    <div class="page-content">
      ...
      <div class="panel">
        <header class="panel-heading">
          ...
          <h3 class="panel-title">List of Defect</h3>
        </header>
        <div class="panel-body">
          
          <div align="right" style="padding-bottom: 15px">
            <button  type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalTambahData" onclick="updateData();" >Add New Data</button>
          </div>

          <!-- Tambah Data -->
          <form role="form" method="post" action="<?php echo base_url()?>index.php/user/tambah_jenis_cacat">
            <div class="modal fade modal-3d-sign" id="modalTambahData" aria-hidden="true" role="dialog" tabindex="-1">
              <div class="modal-dialog modal-simple">
                <div class="modal-content">
                  <div class="modal-header">
                    ...
                    <h4 class="modal-title">Masukan Ceklist</h4>
                  </div>
                  <div class="modal-body">
                    <p>Lengkapi Isian Periksa Cacat Berikut ini !</p>

                    <!-- ISIAN KUISIONER -->
                    <h5>Nama Cacat</h5>
                    <input id="nama_cacat" type="text" class="form-control" placeholder="Cacat [number]" name="nama_cacat"  required>
                    <h5>Deskripsi Jenis Cacat</h5>
                    <textarea id="deskripsi_cacat" name="deskripsi_cacat" class="form-control" placeholder="Masukkan Deskripsi Cacat" required="required" ></textarea>
          ...
          <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
            ...
            <tbody>
              ...
              <?php $no=1; foreach ($jenis_cacat as $d) {
                ...
                <tr>
                  <td><?php echo $no; ?></td>
                  <td><?php echo $d['nama_cacat'] ?></td>
                  <td><?php echo $d['deskripsi_cacat'] ?></td>
                  <td align="right">
                    <button type="button" class="btn btn-sm btn-success" data-toggle="modal" data-target="#exampleModal" onclick="updateData();"
                    <?php echo "data-id_jenis_cacat='".$d['id_jenis_cacat']."'" ?>
                    <?php echo "data-nama_cacat='".$d['nama_cacat']."'" ?>
                    <?php echo "data-deskripsi_cacat='".$d['deskripsi_cacat']."'" ?>

                    data-whatever="apakah"
                    >Edit</button>

                  <button type="button" class="btn btn-sm btn-warning" onclick="hapusData(<?php echo $d['id_jenis_cacat']; ?>);" >Delete</button>
          ...
          <!--Edit-->
          <form role="form" method="post" action="<?php echo base_url()?>index.php/user/ubah_jenis_cacat">
            <div class="modal fade modal-3d-sign" id="exampleModal" aria-hidden="true" aria-labelledby="exampleModalTitle" role="dialog" tabindex="-1">
              <div class="modal-dialog modal-simple">
                <div class="modal-content">
                  <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  ...
                  <div class="modal-body">
                    ...
                    <!-- ISIAN KUISIONER -->
                    <input id="id_jenis_cacat" hidden type="text" name="id_jenis_cacat" value="">
                    <h5>Nama Cacat</h5>
                    <input id="nama_cacat" type="text" class="form-control" placeholder="Cacat [number]" name="nama_cacat"  required>
                    <h5>Deskripsi Jenis Cacat</h5>
                    <textarea id="deskripsi_cacat" name="deskripsi_cacat" class="form-control" placeholder="Masukkan Deskripsi" required="required" ></textarea>
          ...
          <script>
            function updateData(){
              $('#exampleModal').on('show.bs.modal', function (event) {
                  ...
                  var id_jenis_cacat  = button.data('id_jenis_cacat')
                  var nama_cacat      = button.data('nama_cacat')
                  var deskripsi_cacat = button.data('deskripsi_cacat')


                  var modal = $(this)
                  //modal.find('.modal-title').text('SK Yudisium Prodi ')
                  

                  modal.find('.modal-body #id_jenis_cacat').val(id_jenis_cacat)
                  modal.find('.modal-body #nama_cacat').val(nama_cacat)
                  modal.find('.modal-body #deskripsi_cacat').val(deskripsi_cacat)
          ...
          <script>
            function hapusData(apakah){
              //var txt;
                var r = confirm("Apakah anda ingin menghapusnya ?");
                if (r == true) {
                  //txt = "You pressed OK!";
                  window.location.replace("<?php echo base_url()?>index.php/user/hapus_jenis_cacat/"+apakah);
